...
def getRotationMat(roll, pitch, yaw):
    rx = np.array([1., 0., 0., 0., np.cos(deg2rad(roll)), -np.sin(deg2rad(roll)), 0., np.sin(deg2rad(roll)),
                   np.cos(deg2rad(roll))]).reshape((3, 3))
    ry = np.array([np.cos(deg2rad(pitch)), 0., np.sin(deg2rad(pitch)), 0., 1., 0., -np.sin(deg2rad(pitch)), 0.,
                   np.cos(deg2rad(pitch))]).reshape((3, 3))
    rz = np.array(
        [np.cos(deg2rad(yaw)), -np.sin(deg2rad(yaw)), 0., np.sin(deg2rad(yaw)), np.cos(deg2rad(yaw)), 0., 0., 0.,
         1.]).reshape((3, 3))
...
if __name__ == '__main__':
    ...
    # ----------------constants--------------
    path_to_360_images = '/home/kgb/PycharmProjects/ImageTransformer/DeepCalib/SUN360'
    list_360_image_paths = glob.glob(path_to_360_images + '/**/*.jpg', recursive=True)
    print(list_360_image_paths)
    ...
    for image360_path in list_360_image_paths:  # length of your filename list
        ...
        for i in range(1):
            ...
            cv2.imwrite(
                '/home/kgb/PycharmProjects/ImageTransformer/DeepCalib/dataset_discrete/' + name_list[0] + '_f_' + str(f) + '_d_' + str(xi) + '.' + name_list[
                    -1], im)
